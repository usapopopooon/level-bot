...
class MinecraftFishingComboEvent(Base):
    """mc-botが付与したMinecraft内の釣りコンボ報酬監査台帳。"""

    __tablename__ = "minecraft_fishing_combo_events"
    __table_args__ = (
        CheckConstraint(
    ...
    user_id: Mapped[str] = mapped_column(String, nullable=False, index=True)
    minecraft_account_id: Mapped[str] = mapped_column(String(128), nullable=False)
    catch_count: Mapped[int] = mapped_column(BigInteger, nullable=False)
...
class MinecraftVoicePresence(Base):
    ...
    bonus_cursor_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), nullable=False
    )
    updated_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True),
...
class MinecraftXpExchange(Base):
    """サーバーXPをMinecraft内XPへ交換する予約・消費台帳。"""

    __tablename__ = "minecraft_xp_exchanges"
    __table_args__ = (
        CheckConstraint("cost_xp > 0", name="ck_minecraft_xp_exchanges_cost"),
        CheckConstraint("reward_xp > 0", name="ck_minecraft_xp_exchanges_reward"),
        CheckConstraint(
