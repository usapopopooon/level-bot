...
async def test_catalog_is_public_without_login(
    ...
    body = catalog.json()
    assert body["total_cards"] == 527
    assert body["food_cards"] == 219
...
async def test_public_leaderboards_include_names_and_all_ten_categories(
    ...
    assert profile_body["total_cards"] == 527
    assert profile_body["total_sets"] == 55
