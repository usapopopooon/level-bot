...
async def test_catalog_and_images_are_public_without_login(
    ...
    body = catalog.json()
    assert body["total_cards"] == 285
    assert body["food_cards"] == 121
...
async def test_public_leaderboards_include_names_and_all_ten_categories(
    ...
    assert profile_body["total_cards"] == 285
    assert profile_body["total_sets"] == 39
