...
async def test_cafe_capabilities_report_pinned_assets(
    ...
    assert response.json()["catalog_size"] == 508
    assert response.json()["asset_count"] == 510
    ...
    assert response.json()["ranking_category_totals"]["collection"] == 508
    assert response.json()["set_count"] == 53


@pytest.mark.parametrize(
    ("owned", "expected"),
    [(165, False), (166, True), (507, True), (508, False)],
)
