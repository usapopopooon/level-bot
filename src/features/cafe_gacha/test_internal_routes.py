...
async def test_cafe_capabilities_report_pinned_assets(
    ...
    assert response.json()["catalog_size"] == 511
    assert response.json()["asset_count"] == 513
    ...
    assert response.json()["ranking_category_totals"]["collection"] == 511
    assert response.json()["set_count"] == 54


@pytest.mark.parametrize(
    ("owned", "expected"),
    [(165, False), (166, True), (510, True), (511, False)],
)
