...
async def test_cafe_capabilities_report_pinned_assets(
    ...
    assert response.json()["catalog_size"] == 433
    assert response.json()["asset_count"] == 435
    ...
    assert response.json()["ranking_category_totals"]["collection"] == 433
    assert response.json()["set_count"] == 50


@pytest.mark.parametrize(
    ("owned", "expected"),
    [(165, False), (166, True), (432, True), (433, False)],
)
