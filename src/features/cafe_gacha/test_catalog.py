...
def test_catalog_includes_eight_modern_coffeehouse_sweets() -> None:
    expected_by_rarity = {
        "C": {"vanilla-cake-pop"},
        "UC": {
    ...
    tea_keys = {"matcha-cream-frappe"}
    all_keys = set().union(*expected_by_rarity.values())
    ...
    assert (all_keys - food_keys).isdisjoint(FOOD_CARD_KEYS)
...
def test_catalog_includes_three_ancient_japanese_era_foods() -> None:
    expected = {
        "jomon-pottery-nut-soup": ("縄文土器の木の実スープ", "C"),
        "yayoi-jar-red-rice-porridge": ("弥生の甕炊き赤米粥", "UC"),
        "kofun-keyhole-tomb-cake": ("古墳の前方後円墳ケーキ", "R"),
    }

    assert {
        key: (CARDS_BY_KEY[key].name, CARDS_BY_KEY[key].rarity) for key in expected
    } == expected
    assert expected.keys() <= FOOD_CARD_KEYS
    assert CARD_TAGS_BY_KEY["jomon-pottery-nut-soup"] == frozenset({"culture"})
...
def test_catalog_includes_six_european_local_cafe_drinks_as_n() -> None:
    expected_names = {
        "greek-frappe": "ギリシャ・フラッペ",
        "cafe-asiatico": "カフェ・アジアティコ",
...
def test_catalog_includes_five_age_of_sail_provisions_as_n() -> None:
    expected_names = {
        "under-soaked-salt-beef": "塩抜きが足りない塩漬け牛肉",
        "salt-pork-pease-soup": "塩豚と乾燥えんどう豆のスープ",
        "ships-hold-dried-cod": "船倉の干し鱈",
        "hardened-voyage-cheese": "固くなった航海チーズ",
        "barrel-bottom-ale": "樽の底のぬるいエール",
    }
    food_keys = set(expected_names) - {"barrel-bottom-ale"}
...
def test_japanese_local_menu_balances_30_foods_and_30_drinks() -> None:
    ...
    assert {key: card.name for key, card in local_cards.items()} == {
        "hokkaido-imomochi": "いももち",
        "aomori-gapparamochi": "がっぱらもち",
        "iwate-ganzuki": "がんづき",
        "miyagi-zunda-mochi": "ずんだ餅",
        "akita-butter-mochi": "バター餅",
        "fukushima-junen-botamochi": "じゅうねんぼたもち",
        "ibaraki-hoshiimo": "干し芋",
        "tochigi-imo-fry": "いもフライ",
        "gunma-yaki-manju": "焼きまんじゅう",
        "saitama-miso-potato": "みそポテト",
        "chiba-peanut-miso": "落花生みそ",
        "yokohama-sanma-men": "サンマーメン",
        "niigata-poppo-yaki": "ぽっぽ焼き",
        "toyama-tororo-kombu-onigiri": "とろろ昆布おにぎり",
        "fukui-winter-mizu-yokan": "冬の水ようかん",
        "nagano-milk-bread": "牛乳パン",
        "gifu-gohei-mochi": "五平餅",
        "aichi-oni-manju": "鬼まんじゅう",
        "mie-ibara-mochi": "いばら餅",
        "shiga-decchi-yokan": "丁稚羊羹",
        "osaka-takosen": "たこせん",
        "nara-shikishiki": "しきしき",
        "tottori-imo-bota": "いもぼた",
        "shimane-aka-ten": "赤てん",
        "okayama-ebimeshi": "えびめし",
        "hiroshima-gansu": "がんす",
        "dazaifu-umegae-mochi": "梅ヶ枝餅",
        "kumamoto-ikinari-dango": "いきなり団子",
        "oita-yaseuma": "やせうま",
        "okinawa-chinbin": "ちんびん",
        "hokkaido-haskap-soda": "ハスカップソーダ",
        "aomori-apple-juice": "りんごジュース",
        "iwate-wild-grape-juice": "山ぶどうジュース",
        "miyagi-zunda-shake": "ずんだシェイク",
        "yamagata-pine-cider": "パインサイダー",
        "fukushima-peach-nectar": "桃ネクター",
        "tochigi-lemon-milk": "レモン風味乳飲料",
        "kanagawa-shonan-gold-soda": "湘南ゴールドソーダ",
        "yamanashi-grape-juice": "ぶどうジュース",
        "niigata-koji-amazake": "米麹甘酒",
        "fukui-ume-juice": "梅ジュース",
        "shizuoka-tea-latte": "緑茶ラテ",
        "nara-persimmon-leaf-tea": "柿の葉茶",
        "kyoto-hiyashi-ame": "冷やしあめ",
        "wakayama-ume-soda": "梅ソーダ",
        "tottori-nijisseiki-pear-juice": "二十世紀梨ジュース",
        "okayama-white-peach-juice": "白桃ジュース",
        "hiroshima-lemon-squash": "レモンスカッシュ",
        "yamaguchi-summer-orange-juice": "夏みかんジュース",
        "tokushima-sudachi-soda": "すだちソーダ",
        "kagawa-olive-leaf-tea": "オリーブ葉茶",
        "ehime-mikan-juice": "みかんジュース",
        "kochi-yuzu-drink": "ゆずジュース",
        "fukuoka-strawberry-milk": "いちごミルク",
        "nagasaki-sonogi-tea": "そのぎ玉緑茶",
        "kumamoto-shiranui-juice": "不知火ジュース",
        "oita-kabosu-soda": "かぼすソーダ",
        "miyazaki-hyuganatsu-soda": "日向夏ソーダ",
        "kagoshima-brown-sugar-milk": "黒糖ミルク",
        "okinawa-shikuwasa-juice": "シークヮーサージュース",
    }
