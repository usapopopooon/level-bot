...
- **レベル**: 上記 XP の合計 + 項目別。VC は固定換算、TC/リアクションは「重みログ」の有効日で切替
...
- `INTRO_API_BASE_URL` / `INTRO_API_KEY` — level-bot の「チル場所を設定」
  ボタンで保存した内容を移行期間中 intro-bot API にも反映するための設定。
  `INTRO_API_KEY` は intro-bot 側の `INTRO_API_KEY` と一致させる
