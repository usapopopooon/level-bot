...
| `/coffee-market rules` | コーヒー豆相場の遊び方を表示 |
...
管理者が `/coffee-market panel`、`/coffee-market ledger-panel`、
`/coffee-market ranking-panel` をそれぞれ実行したチャンネルへ、相場・公開取引台帳・
週間ランキングの各パネルを個別に投稿する。一つのパネルを設置し直しても、他のパネルの
投稿先は変えない。公開台帳は全員の直近の購入・手動売却・自動売却を一つの固定メッセージで
表示し、取引成立後に更新する。価格はサーバーごとに共通で、日本時間5時に1日1回更新する。
...
| `/coffee-market ledger-panel` | 実行したチャンネルへ公開取引台帳を投稿 |
