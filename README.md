...
レベル計算に使う現在 XP は `獲得 XP - 成功済み交換の消費 XP` で計算する。
...
常設パネルでは1枚引きに加えて10連抽選を利用できる。10連は10回分を1トランザクションで
確定し、10件の画像つき結果をカフェ台帳の1投稿へまとめる。添付10件・Embed 10件を
1回のDiscord API送信で投稿するため、画像を省かず連続投稿も発生しない。既存の1時間
10回上限は共通で、10連にはその時間の空き枠が10回分必要となる。
R・SR・SSRが含まれる場合は、結果投稿の直後にカフェ台帳で抽選者を1回メンションし、
「SR以上」などその抽選で出た最高レアリティに言及する。
...
- **レベル**: 総合は獲得 XP からカラーロール交換の消費 XP を差し引いた現在 XP、項目別は各項目の獲得 XP。VC は固定換算、TC/リアクションは「重みログ」の有効日で切替
