...
スナップショット保存するため、後から商品を改名・削除しても履歴と再配達表示は変わらない。
...
| `/coffee-market ranking` | 本日・過去5日・累計の確定損益ランキングを表示 |
...
カタログは珈琲・紅茶・日本茶・中国茶、世界の飲み物、喫茶店フードや菓子、
歴史的な代用食・飲料、身近な商品の面影を独自にもじった喫茶ネタ、
世相や仕組みを一皿へ落とし込んだ風刺メニュー、世界の喫茶文化と
忘れられた一杯・一皿、史料に残る人物の逸話、現存する食の秘宝、
珈琲と茶の定番銘柄、レプリケート食・宇宙食・培養食、各地の喫茶語、
ローカルモーニング、ネット文化と喫茶店の日常を拾ったお笑いメニュー、
原始・旧石器・新石器時代をたどる先史時代風メニュー、縄文・弥生・古墳時代を
たどる列島古代風メニュー、もの不足のなかのソ連家庭食・共同食堂を描く生活食メニュー、
容器と保存法まで楽しむ世界の即席食、ヨーロッパ各地のローカルなカフェ飲料、
中南米各地の日常に根づくローカルな飲み物、
大航海時代の船倉を支えた保存食と飲み物、
健康維持やダイエットを
気づかう特保風メニュー、紫の虹彩をまとったエンチャント風メニュー、化石・発掘・
オーパーツをたどる超古代文明風メニュー、地上とネザーの菌類を料理やドリンクにした
幻想きのこメニュー、ナイル河畔の日常から王家と星天へ進む古代エジプト風メニュー、
二河の大麦とデーツから粘土板料理、青釉の都と星見へ進む古代メソポタミア風メニュー、
穀物と土器から印章、大浴場、煉瓦都市へ進むインダス文明風メニュー、
黄河の穀物から賈湖の醸し、青銅祭器と玉へ進む古代中華文明風メニュー、
日本・中国・台湾・ヒマラヤ・セイロンなど世界各地の実在する茶種・産地銘柄、
全国の郷土菓子・軽食と地域の果実・茶を使う日本ローカル喫茶メニュー、
作品名・人物・ロゴを使わず料理と場面だけで名作をほのめかす物語メニューを含む
538種類あり、
Nは日常的なお茶に加え、出がらし・代用珈琲・見切り品など、身近で親しみやすい枠として扱う。
URは史料に残る人物の逸話や物語を決定づける一品、幻は現物が収蔵・保存された
一度きりの食の遺物として扱う。
基本排出率は N 65% / HN 24% / R 8% / SR 2.5% / SSR 0.4% / UR 0.08% /
幻 0.02% とし、未収集カードを同一レアリティ内で2倍優遇する。
166種以上の収集後に100回連続でNEWが出なければ、次は未所持カードを確定する。
コレクション棚はレアリティ別に表示し、お気に入り・個別交換も先にレアリティを
選ぶためDiscordの選択肢上限内で操作できる。全カードの画像・基準排出率・解説・
セットメニューは、常設パネルのWeb図鑑リンクから確認できる。
初入手カードは結果に収集数の増加を表示し、新規カード名とNEW演出は結果直後の通知へ
まとめる。R以上または初入手カードが含まれる場合は、カフェ台帳で抽選者を1回
メンションする。レアカードには「SR以上」などその抽選で出た最高レアリティ、初入手には
新規カード名を表示する。
通知対象は抽選した本人のみで、`@here`・`@everyone`・ロールメンションは送信しない。
コレクションの重複交換は「重複を選んでXP交換」と「全重複をXP交換」を
...
修復する。ユーザーはパネルから相手、`1〜5,000 XP` の金額、任意のメッセージを入力し、
...
1回のギフトごとに最初の3,000 XPは非課税とし、超過分の10%を1 XP単位で切り上げて
...
管理者が `/coffee-market panel` と `/coffee-market ranking-panel` を実行したチャンネルへ、
相場とランキングの各パネルを個別に投稿する。ランキングには、本日・今日を含む過去5日・
全期間の確定損益を並べる。各パネルと `/coffee-market ranking` のランキング本体は
BAN済み・退会済み・表示除外ユーザーを除外する。所属状態の変更も監視対象に含めるため、
常設ランキングは取引が発生していない場合でも次回の定期更新で追従する。
Botの通常メッセージとして投稿し、コマンドの実行者は公開表示しない。
`/coffee-market ledger` は実行した
チャンネルを公開取引台帳に指定するだけで、固定パネルは投稿しない。台帳には未投稿の
過去分を含む購入・手動売却・自動売却を、取引ごとに個別の埋め込みメッセージで古い順に
追記する。一つの配置を変更しても、他の投稿先は変えない。価格はサーバーごとに共通で、
日本時間0時・6時・12時・18時の1日4回更新する。相場パネル内で利益の狙い方、
損失の可能性、購入・売却・
自動売却のルールまで説明するため、独立したルール画面は設けない。

`/coffee-market access-role add|remove|list` で豆相場を利用できるロールを管理できる。
複数設定した場合はいずれか1つを持つユーザーとサーバー管理権限を持つユーザーだけが
利用でき、未設定の場合は全員が利用できる。
各ユーザーは相場の更新枠ごとに1回、1〜10袋（1日最大40袋）を購入できる。
購入画面では1〜10袋のセレクトボックスから袋数を選び、内容確認後に確定する。
購入した豆は次の6時間更新後から売却でき、売却回数に制限は設けない。
購入日の7日後の0:00に残っている豆は消失させず、その時点の売値で自動売却する。
手動売却は期限の近い購入分から使う。購入はサーバーXPの消費、手動・自動売却は
サーバーXPの受取として現在レベルとレベルロールへ反映する。購入と売却は、袋数・単価・
合計・取引後XPを本人だけに表示し、確認ボタンを押した場合に確定する。
通常相場はサーバーごとの各相場日に黒字1〜2枠・損益ゼロ1枠・赤字1〜2枠を
順不同で配置し、一方向の相場が続かないようにする。通常時の1袋あたりの損益幅は
2〜9 XPに抑える。赤字枠では売値を直前と現在の買値の両方より低くし、パネル上でも
買値が売値を上回る安値相場を毎日発生させる。
買値の1.8倍以上になるまれな急騰相場と
75%以下になるまれな急落相場は、外れ値として圧縮せず専用の相場ニュースを表示する。
外れ値を含む決定的サンプルでも、次の更新での平均損益はほぼ中立に保ち、7日間の
最良タイミングであっても黒字を保証しない。
約8%の相場枠では、次の更新時の売値が上がるか下がるか、または横ばいかを必ず正しく示す
「市場筋の予測」をニュースへ追加する。予測を保存する同じトランザクションで対象の
次回相場も先行して確定保存し、次の更新までに価格生成ロジックが変わっても予測を外さない。

購入・売却はDiscordの操作ID、期限売却は購入ロットと期限から作る取引IDで冪等に記録する。
相場価格もサーバーIDと相場日から決定的に生成して保存するため、Bot停止や再試行で価格・
売却額が変わらない。Discord UIはDB・XP・レベル同期を直接参照せず、実装非依存の市場
アプリケーション境界だけを呼ぶ。市場DB、XP台帳、レベル同期要求はlevel-bot内の同一
トランザクションで確定する。将来別Botへ移す場合は、この境界へ接続するAPIクライアントを
用意し、取引内のXP接続を別プロセスへ持ち出さない。
...
| `/coffee-market panel` | 実行したチャンネルへコーヒー豆相場パネルを投稿 |
| `/coffee-market ledger` | 実行したチャンネルを追記型の公開取引台帳に指定 |
| `/coffee-market ranking-panel` | 実行したチャンネルへ本日・過去5日・累計ランキングパネルを投稿 |
| `/coffee-market access-role add/remove/list` | コーヒー豆相場の利用ロールを管理 |
...
- **レベル**: 総合は獲得・受取 XP から交換消費とXPギフトの送付額・贈与税を差し引いた現在 XP、項目別は各項目の獲得 XP。VC は固定換算、TC/リアクションは「重みログ」の有効日で切替
  ...
  - まりも復活に使う「苔コーラ」は、カフェ・コレクションの最初の1枚を保護して重複分だけを消費し、XP復活とは排他的にイベントID単位で一度だけ台帳へ記録
  - カード棚の「保護カードを設定」で選んだカード種類は、XP・カフェメダルへの個別／全重複交換から除外する。保護は将来増えた重複にも継続し、明示的なアイテム使用（苔コーラでの復活）は妨げない
  - Minecraftアイテムガチャは通常 `100 XP`、R以上確定 `1,000 XP`。「おまかせ」「資源・採掘」「冒険」「装備・強化」のカテゴリと引き方を合わせて日本時間の日付ごとに3回まで。カテゴリも冪等な予約へ固定し、配布前に予約して成功後に確定する。Minecraftが明確に拒否した場合だけ予約を取り消し、成否不明時は二重配布防止のため消費を維持する
  - Minecraft資源売却はエメラルド64個を `500 XP` で、土・砂・砂岩・深層岩・深層岩の丸石・凝灰岩を64個単位の所定レートで受け付け、合計で1人1日 `3,000 XP`（日本時間）まで。日次枠を予約し、Paper側の回収・保存が成功した後だけサーバーXPへ加算する。同じ操作IDの再試行は二重加算しない
  - コーヒー豆相場は購入額を消費XP、手動・期限売却額を受取XPとして扱い、売買のたびに総合レベルを再計算する
