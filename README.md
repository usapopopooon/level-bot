...
| `/coffee-market ranking` | 今週の確定損益ランキングを表示 |
...
相場と週間ランキングの各パネルを個別に投稿する。`/coffee-market ledger` は実行した
...
日本時間5時に1日1回更新する。相場パネル内で利益の狙い方、損失の可能性、購入・売却・
...
購入日の7日後、5:00まで残っている豆は消失させず、その時点の売値で自動売却する。
...
| `/coffee-market ranking-panel` | 実行したチャンネルへ週間ランキングパネルを投稿 |
