...
価格曲線は勝ちやすさを残しつつ損失も現実的に起こるようにし、本来なら次の更新で
黒字になる相場の一部は、その購入時の買値と次の売値が同額になる。決定的サンプル上で、
次の相場更新で黒字になる率は約60%、損益ゼロになる率は17〜20%、
赤字になる率は20〜24%に保つ。
通常相場の買値と売値の差は少し圧縮する一方、買値の1.8倍以上になるまれな急騰相場と
75%以下になるまれな急落相場は、外れ値として圧縮せず専用の相場ニュースを表示する。
7日間のうち一度でも黒字になる率は99%以上とし、必ず利益になる仕様にはしない。
